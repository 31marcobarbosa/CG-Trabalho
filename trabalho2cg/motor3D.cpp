...
void renderScene(void) {
	...
	for(j = 0; j < aplicacao.size(); j++){
		...
		glPushMatrix();
		Transformacao t = aplicacao[j]->getTransformacao();

		//glRotatef();
		glTranslatef(t.getTranslacao().getTX(),t.getTranslacao().getTY(),t.getTranslacao().getTZ());
...
void parseNivelado(XMLElement *grupo, Transformacao *t){
	...
	for(;(strcmp(transformacao->Value(),"modelos")!=0);transformacao = transformacao -> NextSiblingElement()) {
		 ...
		 if (strcmp(transformacao->Value(), "translacao") == 0){
				...
				tr = new Translacao(transX, transY, transZ);
	...
	trans = new Transformacao(*tr,*esc);
